...
description: >
  ...
  load this skill alongside the more specific skills (compose, kotlin-flows,
  android-retrofit, android-llm-streaming, android-secure-credentials,
...
- **DI:** Hilt + KSP (or `koin` when the project uses Koin). **Async:** Coroutines/Flow — no `LiveData` in new code. **JSON:** `kotlinx.serialization`. **Images:** Coil.
...
- **Errors:** mapped to a domain type at the repository boundary — platform exceptions never leak past it; UI state explicitly models loading / success / error (see `android-data-layer`).

## Skill routing

These skills are vendored locally into this repo's `.claude/skills/`, so load them
by their **plain name** (`compose`, `koin`, …). Upstream's `android-skills:`
marketplace prefix does not apply in this layout.

| For | Load |
|---|---|
| Compose detail — stability, `remember`, modifiers, side effects, lists, animation, navigation | `compose` |
| Compose deep-dives — state ownership & effects, recomposition/stability, component API shape, motion, UI tests | `compose-state-and-effects`, `compose-performance`, `compose-component-design`, `compose-animations`, `compose-ui-testing-patterns` |
| M3 UX — touch targets, adaptive/foldable layouts, accessibility & M3-compliance audit | `android-ux` |
| Coroutines & Flow — operators, `Channel` vs `SharedFlow`, structured concurrency | `kotlin-coroutines`, `kotlin-flows` |
| Kotlin API shape & control flow — ownership, value classes, `when` exhaustiveness | `kotlin-api-design`, `kotlin-control-flow` |
| Repository / data layer + error model | `android-data-layer` |
| Networking (Android) | `android-retrofit` |
| **Streaming LLM responses — SSE framing, partial deltas, cancellation** | `android-llm-streaming` |
| **API keys / provider credentials** | `android-secure-credentials` |
| Preferences / typed local storage | `datastore` |
| Testing | `android-testing` |
| DI with Koin | `koin` |
| Build logic / convention plugins | `android-gradle-logic` |
| Build speed, kapt → KSP | `gradle-build-performance` |
| Debugging — Logcat, crashes, ANRs, profiling | `android-debugging` |
| AOSP / AndroidX source lookup | `android-source-search` |
| Multi-module visibility & module boundaries | `modularization` |
| Talking to an OpenClaw Gateway | `openclaw-gateway-client` |

Not vendored (this is an Android-only, non-KMP app): `kmp-ktor`, `kmp-boundaries`,
`paging`, `coil-compose`, `rxjava-migration`, `pdf-annotations`. If the project
later goes multiplatform or needs them, pull them from upstream rather than
improvising.
...
> **Kotlin 2.4+:** collapse the `_uiState`/`uiState` pair with explicit backing fields (`val uiState: StateFlow<FooUiState>` + `field = MutableStateFlow(…)`) — `uiState` only, never the effects `Channel`. Full idiom + version gate: `kotlin-flows`.
