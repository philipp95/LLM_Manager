...
Default to `launch { _events.emit(e) }` — it suspends until each *existing* subscriber is ready, so an effect is not dropped on a slow collector. **It does not save you when there are zero subscribers:** on a default `MutableSharedFlow()` (replay 0, no buffer), `emit` with no subscribers returns immediately and the value is discarded — a backgrounded screen that tore down its collector loses the effect. That is exactly why the house default for one-shot effects is `Channel(Channel.BUFFERED).receiveAsFlow()`, not `SharedFlow` (see `android-dev`). `tryEmit()` on that same default flow **silently drops** when no subscriber is ready; adding `extraBufferCapacity = 1` only moves the cliff (a second rapid emission while the buffer is full returns `false` and is dropped with no error). Use `tryEmit` only for miss-tolerable effects (tooltip, sound).
